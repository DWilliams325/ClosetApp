...
// Draft (in-progress) state so users don’t lose work
...
// ---------- Image compression utilities (mobile-friendly) ----------
...
function canvasToJPEGDataURL(canvas, quality = 0.8) {
...
// Use video frame directly -> dataURL
function compressVideoFrameToDataURL(videoEl){
  const vw = videoEl.videoWidth, vh = videoEl.videoHeight;
  if (!vw || !vh) return null;
  // scale down to 1280 on long edge
  const max = 1280;
...
// Load draft (if any) AFTER categories exist
loadDraft();
validateForm();
...
photoInput.addEventListener('change', async (e) => {
  ...
  if (files.length > 1) {
    ...
    for (let i=0;i<files.length;i++){
      ...
      items.push({ id:(now+i).toString(), name: baseName || `Item ${i+1}`, category, imageDataURL });
    }
    saveItems();
...
function renderList() {
  ...
  data.forEach(it => {
    ...
    editBtn.className = 'btn btn-edit'; editBtn.textContent = 'Edit Photo';
...
// ---------- Share link + modal (existing optional) ----------
if (shareBtn) {
...
function stopSpeedCamera(){
  if (speedStream){
    speedStream.getTracks().forEach(t=>t.stop());
    speedStream = null;
  }
...
function updateSpeedUI(){
  ...
  speedQueue.slice(-30).forEach((q,i)=>{
    const img = document.createElement('img');
...
speedUndo.addEventListener('click', ()=>{
  speedQueue.pop();
  updateSpeedUI();
});
...
speedShutter.addEventListener('click', async ()=>{
  ...
  // yield so UI stays responsive on phones if user spams shutter
...
speedSave.addEventListener('click', ()=>{
  ...
  const t0 = Date.now();
  const base = new Date().toISOString().slice(0,19).replace(/[:T]/g,'-'); // yyyy-mm-dd-hh-mm-ss
